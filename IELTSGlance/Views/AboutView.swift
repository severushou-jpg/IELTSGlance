        ...
        HStack(alignment: .top, spacing: 14) {
            ...
            GroupBox("About / Credits") {
                VStack(alignment: .leading, spacing: 6) {
                    Text("IELTS Glance \(version)")
                        .font(.callout.weight(.semibold))
                    Text("代码：MIT License")
                    Text("词库：ECDICT MIT + Open English WordNet CC BY 4.0")
                    if let repositoryURL = URL(string: "https://github.com/severushou-jpg/IELTSGlance") {
                        Link("GitHub：severushou-jpg/IELTSGlance", destination: repositoryURL)
                    }

...
final class AppWordStore {
    ...
    private(set) var preferences: GREGlancePreferences
    ...
    private var selectedWords: [GREWord] {
        snapshot.words(selectedPackIDs: preferences.selectedPackIDs)
    }

    private func updatePreferences(_ transform: (inout GREGlancePreferences) -> Void) {

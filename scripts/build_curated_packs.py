#!/usr/bin/env python3
"""Build 15 deterministic GRE packs from licensed ECDICT and Open English WordNet data.

Selection rule: keep single English headwords tagged ``gre`` but not ``ielts`` in
ECDICT, require useful Open English WordNet lexical data, then rank by the best
available BNC/COCA-style frequency rank. The committed resource is generated
offline; the app never downloads or invokes Python at runtime.
"""
...
DEFAULT_SEED = ROOT / "data" / "gre_words_seed.tsv"
DEFAULT_OUTPUT = ROOT / "Shared" / "Resources" / "gre_word_packs.json"
DEFAULT_MANIFEST = ROOT / "data" / "curated_words.txt"
DEFAULT_SIGNALS = ROOT / "data" / "curation_signals"
WORD_PATTERN = re.compile(r"^[a-z][a-z-]*$")
POS_MAP = {"a": "adj.", "s": "adj.", "r": "adv.", "n": "n.", "v": "v."}


def stable_id(word: str) -> str:
    return "gre-" + re.sub(r"[^a-z0-9]+", "-", word.lower()).strip("-")
...
def build_words(
    ...
    with ecdict_path.open(encoding="utf-8", errors="strict", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            if "gre" not in tags or "ielts" in tags or not WORD_PATTERN.fullmatch(word):
                continue
            ...
            # Exclude the very common core-English band even when a source tag
            # happens to include GRE. This keeps the collection meaningfully
            # above typical IELTS/general-vocabulary material while retaining
            # words that still occur often enough in broad English corpora.
            if best_rank < 12_000 or best_rank >= 1_000_000:
                continue
            item = {
                "id": stable_id(word),
                "word": word,
            ...
            editorial_priority = 0 if authored else (1 if word in signals else 2)
            score = (editorial_priority, best_rank, positive_rank(row.get("frq", "")), word)
...
def make_packs(words: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index in range(15):
        start = index * 100
        packs.append({
            "id": f"gre-pack-{index + 1:02d}",
            "name": f"GRE 进阶 {index + 1:02d}",
...
def main() -> int:
    ...
    if DEFAULT_SIGNALS.exists():
        signal_paths.extend(sorted(DEFAULT_SIGNALS.glob("*.txt")))
    signals: set[str] = set()
    ...
    if args.refresh_selection:
        ...
        args.selection_manifest.write_text(
            "# GRE-tagged, non-IELTS ECDICT headwords; corpus rank >= 12,000.\n"
